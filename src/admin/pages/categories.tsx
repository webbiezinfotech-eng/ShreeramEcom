...
const Categories: React.FC = () => {
  ...
  const [form, setForm] = useState<{ name: string; slug: string; image: File | null; status: 'active' | 'inactive' }>({ name: "", slug: "", image: null, status: 'active' });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.image) {
        ...
        if (modalMode === 'edit' && selectedCategory) {
          await categoriesAPI.update(selectedCategory.id, { name: form.name.trim(), slug, status: form.status });
        } else {
          await categoriesAPI.create({ name: form.name.trim(), slug, status: form.status });
        }
      }
      
      setIsModalOpen(false);
      setForm({ name: "", slug: "", image: null, status: 'active' });
  ...
  const openEditModal = (cat: Category) => {
    ...
    setForm({ name: cat.name, slug: cat.slug || "", image: null, status: cat.status || 'active' });
          ...
          <div className="bg-white w-full max-w-md rounded-xl shadow-xl p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">Inactive categories won't be shown on the website</p>
              </div>
              <div>
